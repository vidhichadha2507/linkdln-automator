...
// Vercel serverless has a read-only filesystem — backups are stored in the DB
// Each backup is a SystemSetting row with key = "backup_<timestamp>"
...
export async function createBackupSnapshot(): Promise<{ filename: string; size: number }> {
  ...
  // Store in DB — key = "backup_<timestamp>", value = JSON
  await prisma.systemSetting.upsert({
    where: { key: filename },
    update: { value: fileContent },
    create: { key: filename, value: fileContent }
  });
...
export async function listBackupSnapshots(): Promise<Array<{ filename: string; size: number; createdAt: string }>> {
  try {
    ...
    return rows.map(row => ({
      filename: row.key,
      size: Buffer.byteLength(row.value, "utf-8"),
...
export async function getBackupContent(filename: string): Promise<string | null> {
  if (!filename.startsWith(BACKUP_KEY_PREFIX) || filename.includes("..") || filename.includes("/")) {
    return null;
  }
  try {
    const row = await prisma.systemSetting.findUnique({ where: { key: filename } });
...
export async function restoreBackupSnapshot(filename: string, mode: "replace" | "add"): Promise<{ success: boolean; message?: string }> {
  ...
  try {
    const row = await prisma.systemSetting.findUnique({ where: { key: filename } });
    if (!row) return { success: false, message: "Backup not found in database." };

    const parsed = parseDates(JSON.parse(row.value));
    ...
    if (mode === "replace") {
      ...
      for (const sa of emailAlgorithms) {
        ...
        if (match) { algorithmIdMap[sa.id] = match.id; }
        else {
          const created = await prisma.emailAlgorithm.create({ data: { key: sa.key, patternTemplate: sa.patternTemplate, description: sa.description, example: sa.example } });
      ...
      for (const sc of companies) {
        ...
        if (match) { companyIdMap[sc.id] = match.id; }
        else {
          const created = await prisma.company.create({ data: { name: sc.name, normalizedName: sc.normalizedName, domain: sc.domain, domainConfidence: sc.domainConfidence, domainSource: sc.domainSource, researchReason: sc.researchReason } });
      ...
      for (const sca of companyEmailAlgorithms) {
        ...
        if (mappedCompanyId && mappedAlgorithmId) {
          ...
          if (!match) await prisma.companyEmailAlgorithm.create({ data: { companyId: mappedCompanyId, algorithmId: mappedAlgorithmId, hitCount: sca.hitCount, missCount: sca.missCount, verificationSuccessCount: sca.verificationSuccessCount, bounceCount: sca.bounceCount, confidenceScore: sca.confidenceScore, lastVerifiedAt: sca.lastVerifiedAt, rank: sca.rank } });
        }
      }
      ...
      for (const sl of leads) {
        ...
        const match = existingLeads.find(l => (sl.linkedinUrl && l.linkedinUrl === sl.linkedinUrl) || (l.companyId === companyId && l.fullName.toLowerCase() === sl.fullName.toLowerCase()));
        if (match) { leadIdMap[sl.id] = match.id; }
        else {
          const created = await prisma.lead.create({ data: { fullName: sl.fullName, firstName: sl.firstName, middleName: sl.middleName, lastName: sl.lastName, companyId, linkedinUrl: sl.linkedinUrl, headline: sl.headline, source: sl.source, status: sl.status } });
      ...
      for (const scand of emailCandidates) {
        ...
        if (leadId && companyId && algorithmId) {
          ...
          if (match) { candidateIdMap[scand.id] = match.id; }
          else {
            const created = await prisma.emailCandidate.create({ data: { leadId, companyId, algorithmId, email: scand.email, syntaxValid: scand.syntaxValid, mxValid: scand.mxValid, verifierProvider: scand.verifierProvider, verifierStatus: scand.verifierStatus, verifierScore: scand.verifierScore, isCatchAll: scand.isCatchAll, selected: scand.selected } });
      ...
      for (const st of templates) {
        ...
        if (match) { templateIdMap[st.id] = match.id; }
        else {
          const created = await prisma.template.create({ data: { name: st.name, subject: st.subject, body: st.body, isDefault: st.isDefault } });
      ...
      for (const scs of campaignStates) {
        ...
        if (leadId && candidateId) {
          ...
          if (!match) await prisma.campaignState.create({ data: { leadId, candidateId, status: scs.status, jobLink: scs.jobLink, jobId: scs.jobId, resumePath: scs.resumePath, resumeName: scs.resumeName, scheduledFor: scs.scheduledFor, lastSentAt: scs.lastSentAt, followupCount: scs.followupCount, followupIntervalMinutes: scs.followupIntervalMinutes, maxFollowups: scs.maxFollowups, subject: scs.subject, body: scs.body, respectTiming: scs.respectTiming, isPaused: scs.isPaused, skipBounceMonitor: scs.skipBounceMonitor, roleName: scs.roleName, templateId } });
        }
      }

      for (const se of emailEvents) {
        const candidateId = candidateIdMap[se.candidateId];
        if (candidateId) await prisma.emailEvent.create({ data: { candidateId, eventType: se.eventType, provider: se.provider, rawPayload: se.rawPayload } });
      }

      for (const sse of suppressionEntries) {
        const match = existingSuppressionEntries.find(e => (sse.email && e.email === sse.email) || (sse.domain && e.domain === sse.domain));
        if (!match) await prisma.suppressionEntry.create({ data: { email: sse.email, domain: sse.domain, reason: sse.reason, source: sse.source } });
      }

      for (const sgc of googleCredentials) {
        const match = existingGoogleCredentials.find(c => c.key === sgc.key);
        if (!match) await prisma.googleCredentials.create({ data: { key: sgc.key, refreshToken: sgc.refreshToken } });
      }

      for (const sapp of applications) {
        const match = existingApplications.find(a => a.companyName.toLowerCase() === sapp.companyName.toLowerCase() && a.role.toLowerCase() === sapp.role.toLowerCase());
        if (!match) await prisma.application.create({ data: { companyName: sapp.companyName, role: sapp.role, jobLink: sapp.jobLink, jobId: sapp.jobId, status: sapp.status } });
      }
    }
    ...
    return { success: false, message: error instanceof Error ? error.message : "Unknown restore error" };
  }
}
...
export async function deleteBackupSnapshot(filename: string): Promise<{ success: boolean; message?: string }> {
  ...
  try {
    await prisma.systemSetting.delete({ where: { key: filename } });
    return { success: true };
  } catch (error) {
    console.error("Failed to delete backup snapshot:", error);
    return { success: false, message: error instanceof Error ? error.message : "Failed to delete backup" };
  }
}
